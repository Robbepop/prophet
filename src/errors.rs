...
impl Error {
    ...
    pub(crate) fn zero_layer_size() -> Error {
        Error {
            ...
            message: String::from(
                "Cannot create a layer size representing zero (0) neurons.",
            ),
            annotation: None,
        }
    }
    ...
    pub(crate) fn zero_inputs_weights_matrix() -> Error {
        Error {
            ...
            message:
                "Tried to create a WeightsMatrix for zero inputs. Must be at least one!"
                    .to_owned(),
            annotation: None,
        }
    }
    ...
    pub(crate) fn zero_outputs_weights_matrix() -> Error {
        Error {
            ...
            message:
                "Tried to create a WeightsMatrix for zero outputs. Must be at least one!"
                    .to_owned(),
            annotation: None,
        }
    }
    ...
    pub(crate) fn attempt_to_create_zero_sized_buffer() -> Error {
        Error {
            ...
            message:
                "Attempted to create a buffer with a length (dimension) of zero (1)."
                    .to_owned(),
            annotation: None,
        }
    }
    ...
    pub(crate) fn unmatching_user_provided_bias_value(
        expected: f32,
        actual: f32,
    ) -> Error {
    ...
    pub(crate) fn empty_sample_collection() -> Error {
        Error {
            ...
            message: "Tried to create a SampleCollection from an empty set of samples!"
                .to_owned(),
            annotation: None,
        }
    }
    ...
    pub(crate) fn unmatching_sample_input_len(
        required_len: usize,
        actual_len: usize,
    ) -> Error {
        Error{
    ...
    pub(crate) fn unmatching_sample_expected_len(
        required_len: usize,
        actual_len: usize,
    ) -> Error {
        Error{
    ...
    pub(crate) fn mse_invalid_empty_expected_buffer() -> Error {
        Error {
            ...
            message:
                "Tried to calculate a MeanSquaredError with an empty expected buffer!"
                    .to_owned(),
            annotation: None,
        }
    }
    ...
    pub(crate) fn mentor_builder_invalid_argument(
        arg: MentorBuilderInvalidArgument,
    ) -> Error {
        Error {
