...
mod tests {
    ...
    mod base {
        ...
        fn partial_eq_true() {
            fn assert_for_marker<B: Unbiased>() {
                ...
                let same_as_just_two =
                    AnyBuffer::<B>::from_raw(two_vals.clone()).unwrap();
            ...
            fn assert_for_biased() {
                fn assert_for_marker<B: Biased>() {
                    fn assert_for_buffer<B: Biased>(arr_with_bias: Array1<f32>) {
                        let buf =
                            AnyBuffer::<B>::from_raw_with_bias(arr_with_bias.clone())
                                .unwrap();
                        assert_eq!(
                            buf.view(),
                            AnyView::<B> {
                    ...
                    fn assert_for_view<B: Biased>(arr_with_bias: Array1<f32>) {
                        let view =
                            AnyView::<B>::from_raw_with_bias(&arr_with_bias).unwrap();
                        assert_eq!(
                            view.view(),
                            AnyView::<B> {
                                data: arr_with_bias.view(),
                    ...
                    fn assert_for_view_mut<B: Biased>(arr_with_bias: Array1<f32>) {
                        ...
                        let view_mut = AnyViewMut::<B>::from_raw_with_bias(
                            &mut cloned_arr_with_bias,
                        )
                        .unwrap();
                        assert_eq!(
                            view_mut.view(),
            ...
            fn assert_for_unbiased() {
                fn assert_for_marker<B: Unbiased>() {
                    ...
                    fn assert_for_view_mut<B: Unbiased>(arr: Array1<f32>) {
                        ...
                        let view_mut =
                            AnyViewMut::<B>::from_raw(&mut cloned_arr).unwrap();
                        assert_eq!(
                            view_mut.view(),
                            AnyView::<B> {
                                data: arr.view(),
            ...
            fn assert_for_biased() {
                fn assert_for_marker<B: Biased>() {
                    ...
                    fn assert_for_view_mut<B: Biased>(arr_with_bias: Array1<f32>) {
                        let mut arr_with_bias = arr_with_bias;
                        let mut cloned_arr_with_bias = arr_with_bias.clone();
                        let mut view_mut = AnyViewMut::<B>::from_raw_with_bias(
                            &mut cloned_arr_with_bias,
                        )
                        .unwrap();
                        assert_eq!(
        ...
        fn iter() {
            ...
            fn assert_for_unbiased<B: Unbiased>() {
                let buf =
                    AnyBuffer::<B>::from_raw(vec![1.0, 2.0, 42.0, 13.37, -1.0]).unwrap();
                let mut iter = buf.iter();
                assert_eq!(iter.next(), Some(&1.0));
                assert_eq!(iter.next(), Some(&2.0));
                assert_eq!(iter.next(), Some(&42.0));
        ...
        fn iter_mut() {
            fn assert_for_biased<B: Biased>() {
                ...
                assert_eq!(iter.next(), Some(&mut 42.0));
                assert_eq!(iter.next(), Some(&mut 13.37));
                assert_eq!(iter.next(), Some(&mut B::DEFAULT_BIAS_VALUE));
                assert_eq!(iter.next(), None);
            }
            fn assert_for_unbiased<B: Unbiased>() {
                let mut buf =
        ...
        fn data() {
            fn assert_for_marker<B: Unbiased>() {
                let buf =
                    AnyBuffer::<B>::from_raw(vec![1.0, 2.2, 42.0, 13.37, -1.0]).unwrap();
                assert_eq!(
                    buf.data(),
    ...
    mod biased {
        ...
        fn from_raw_with_bias_fail() {
            fn assert_for_marker<B: Biased>() {
                ...
                }
                {
                    ...
                    let expected =
                        Err(Error::too_few_values_provided_for_buffer_creation(2, 1));
                    assert_eq!(actual, expected);
                }
                {
        ...
        fn unbias() {
            fn assert_for_marker<B: Biased>() {
                ...
                let biased_buf =
                    AnyBuffer::<B>::from_raw_with_bias(biased_values).unwrap();
                let unbiased_buf =
                    AnyView::<B::Unbiased>::from_raw(&unbiased_values).unwrap();
        ...
        fn into_unbias() {
            fn assert_for_marker<B: Biased>() {
                ...
                let unbiased_values = vec![42.0, 1337.0];
                let biased_view =
                    AnyView::<B>::from_raw_with_bias(&biased_values).unwrap();
                let unbiased_view =
                    AnyView::<B::Unbiased>::from_raw(&unbiased_values).unwrap();
        ...
        fn into_unbias_mut() {
            fn assert_for_marker<B: Biased>() {
                ...
                let mut unbiased_values = vec![42.0, 1337.0];
                let biased_view =
                    AnyViewMut::<B>::from_raw_with_bias(&mut biased_values).unwrap();
                let unbiased_view =
                    AnyViewMut::<B::Unbiased>::from_raw(&mut unbiased_values).unwrap();
    ...
    mod unbiased {
        ...
        fn assign_ok() {
            fn assert_for_marker<B: Unbiased>() {
                let mut buf_before =
                    AnyBuffer::<B>::from_raw(vec![42.0, 1337.0, 77.7]).unwrap();
                ...
                assert_eq!(
                    ...
                    Err(
                        Error::unmatching_buffer_sizes(buf_before.dim(), buf_less.dim())
                            .with_annotation(
                                "Occured in unbiased Buffer::assign method."
                            )
                    )
                );
                assert_eq!(
    ...
    mod error_signal {
        ...
        fn reset_to_zeros() {
            ...
            let zeros =
                BiasedErrorSignalBuffer::from_raw_with_bias(vec![0.0, 0.0, 0.0]).unwrap();
