...
impl TrainCondition for BelowRecentMSE {
    #[inline]
    fn evaluate(&mut self, stats: &TrainingState) -> bool {
        self.rmse = (1.0 - self.momentum) * stats.latest_mse().to_f32()
            + self.momentum * self.rmse;
...
mod tests {
    ...
    mod time_elapsed {
        ...
        fn eval_false() {
            assert_eq!(
                TimeElapsed(time::Duration::from_secs(1000))
                    .evaluate(&time_elapsed_ctx()),
                false
            )
        }
    ...
    mod below_recent_mse {
        ...
        fn latest_mse_context<MSE: Into<MeanSquaredError>>(
            latest_mse: MSE,
        ) -> DummyContext {
