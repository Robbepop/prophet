...
mod details {
	...
	/// Identity: *ƒ(x) = x*
	pub fn identity<F: NdFloat>(x: F) -> F { x }
	/// Derivation of the Identity: *ƒ(x) = 1*
	pub fn identity_dx<F: NdFloat>(_: F) -> F { F::one() }
	...
	/// SoftSign: *ƒ(x) = x ⋅ (1 + |x|)⁻¹*
	pub fn softsign<F: NdFloat>(x: F) -> F { x / (F::one() + x.abs()) }
	/// Derivation of SoftSign: *ƒ(x) = ( (1 + |x|)² )⁻¹*
	pub fn softsign_dx<F: NdFloat>(x: F) -> F { let dx = F::one() + x.abs(); F::one() / (dx*dx) }

	/// ReLU:  
	/// *ƒ(x) = 0* **if** *x < 0*  
	/// *ƒ(x) = x* **else**
	pub fn relu<F: NdFloat>(x: F) -> F { if x < F::zero() { F::zero() } else { x } }

	/// Derivation of ReLU:  
	/// *ƒ(x) = 0* **if** *x < 0*  
	/// *ƒ(x) = 1* **else**
	pub fn relu_dx<F: NdFloat>(x: F) -> F { if x < F::zero() { F::zero() } else { F::one() } }
