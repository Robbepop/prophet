    ...
    Sampler: SampleGen,
{
    ...
    fn from_builder(
        builder: InitializingMentor<Sampler, StopWhen, LogWhen>,
    ) -> Result<Self> {
        let ctx = Context::new(
            builder
                .epoch_len
    ...
    mod context {
        ...
        fn update_mse() {
            fn assert_for(mse: MeanSquaredError) {
                let mut ctx =
                    Context::new(1, 1, LearnRate::from(0.1), LearnMomentum::from(0.5));
                ctx.update_mse(mse);
        ...
        fn is_batch_finished() {
            {
                let mut ctx =
                    Context::new(1, 1, LearnRate::from(0.5), LearnMomentum::from(0.5));
            ...
            {
                let mut ctx =
                    Context::new(1, 2, LearnRate::from(0.5), LearnMomentum::from(0.5));
                assert_eq!(ctx.is_batch_finished(), true);
                ctx.next();
